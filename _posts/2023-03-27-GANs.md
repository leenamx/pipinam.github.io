...
至此，我们得到了GAN的价值函数：
$$
\min_{G} \max_{D} V(G,D) = E_{x \sim p_{data}}[\log{D(x)}] + E_{z \sim p_{Z}}[\log{(1-D(G(z)))}] \tag{6}
$$
...
$KL$ 散度又称 $KL$ 距离，相对熵。KL散度是描述两个概率分布 $A$ 和 $B$ 之间差异的一种方法。用于衡量两个分布的偏离程度，如果两个分布完全匹配，那么 $KL(A||B)=0$，否则其取值应该在0～$\infty$之间。$KL$ 散度越小，两个分布之间的相似度越高。

并且简单介绍一下 $KL$ 散度的两个基本性质：

1、非负性
$$
KL(A \mid \mid B) \geq 0
\tag{13}
$$
