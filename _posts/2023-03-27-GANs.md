...
V(G,D^{*}) =  -\log{2} \int_{x} \left[ p_{data}(x) + p_{g}(x) \right] dx 
        + \int_{x} \left[ p_{data}(x) \log{\frac{p_{data}(x)}{\frac{p_{data}(x)+p_{g}(x)}{2}}} \right]
        + \int_{x} \left[ p_{g}(x) \log{\frac{p_{g}(x)}{\frac{p_{data}(x)+p_{g}(x)}{2}}} \right]
\end{aligned}
\tag{}
$$
...
最优生成器 $G^{*}$ 在目标函数 $V(G,D^{*})$ 最小时取得。由于 $JS$ 散度的非负性，因此当且仅当 $p_{data}=p_{g}$ 时 $V(G,D^{*})$ 取得最小值 $-\log4$，此时 $JS(p_{data} \mid \mid p_{g})=0$，即生成器完美的学习到了真实数据的分布。
